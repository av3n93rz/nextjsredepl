...
const AdminDrawer = ({user, isMobile=false}) => {
  ...
  return (
    <div>
          {isMobile ? (<MenuIcon onClick={toggleDrawer(true)} className={classes.hamburgerIcon}/>):(<Button variant="contained" color="primary" className={classes.buttons} onClick={toggleDrawer(true)}>{user}</Button>)}
